...
def from_screen_to_cells(screen_x, screen_y):
    x_cell = int(screen_x / GRIDSIZE)
    y_cell = int(screen_y / GRIDSIZE)
    #print(f"{screen_x}, {screen_y} -> {x_cell}, {y_cell}, ug: {len(uniform_grid[0])} x {len(uniform_grid)}")
...
class Mob:
    ...
    def check_collisions(self):
        ...
        x_cell, y_cell = from_screen_to_cells(self.pos.x, self.pos.y)
...
def update_mobs(mobs):
    ...
    for mob in mobs:
        ...
        # update  uniform_grid
        x_cell_mob, y_cell_mob = from_screen_to_cells(mob.pos.x, mob.pos.y)
