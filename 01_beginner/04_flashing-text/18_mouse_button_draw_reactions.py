...
def draw():
    ...
    if clicked_button:
        if (clicked_button == mouse.LEFT and text_indx != color_indx) or\
            ...
            screen.draw.text("x", center=mouse_pos, fontsize=220, color="red", owidth=0.2, ocolor=(200, 0, 0))
        elif (clicked_button == mouse.LEFT and text_indx == color_indx) or\
                (clicked_button == mouse.RIGHT and text_indx != color_indx):
            screen.draw.text("√", center=mouse_pos, fontsize=220, color="green", owidth=0.2, ocolor=(0, 200, 0))
