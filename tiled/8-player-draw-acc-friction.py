...
class Player:
    ...
    def __repr__(self):
        return f"Player({self.rect.center}, v:({self.vel.x:.2f}, {self.vel.y:.2f}), a:({self.acc.x:.2f}, {self.acc.y:.2f}))"
...
start_y = (screen.get_height() - map_height_px) // 2

player = Player(screen.get_width() // 2, map_height_px - (24 // 2) + 4, 24, 24)
